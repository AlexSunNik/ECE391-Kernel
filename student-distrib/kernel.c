...
void entry(unsigned long magic, unsigned long addr) {
    ...
    /* initialize paging */
    init_paging();
    /* initalize video_mem ptr at 0xb8000 both physical and virtual */
    ...
    /* Initialize devices and interrupts */
    enable_irq(1);  // rtc is on 1
    rtc_init();
    enable_irq(8);  // keyboard is on 8

    /* initialize cursor */
    enable_cursor();

    /* initialize file system */
    init_file_system(FS_ADDR);
...
#if RUN_TESTS
    /* Run tests */
    launch_tests();   // comment out for test load gdt
#else
